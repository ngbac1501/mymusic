...
export const AlbumPage: React.FC = () => {
  ...
  const { data: album, isLoading } = useQuery<Playlist>({
    queryKey: ['album', id],
  ...
  const handlePlayAll = () => {
    if (album?.song?.items && album.song.items.length > 0) {
      setQueue(album.song.items);
      setCurrentIndex(0);
      setCurrentSong(album.song.items[0]);
    }
  };
  ...
  if (!album) {
    return (
      <div className="p-6 text-center">
        <p className="text-gray-500 dark:text-gray-400">Không tìm thấy album</p>
      </div>
    );
  }
